...
class Wildfire
{
    ...
    public function delete($table, $delimiters = array())
    {
        ...
        $this->_ci->db->where($delimiters);
        return ($this->_ci->db->delete($table)) ? TRUE : FALSE;
    }
    ...
    public function get_all($table, $delimiters = array())
    {
        ...
        if (isset($delimiters['per_page']) && $delimiters['per_page'] != NULL) {
            ...
            $this->_ci->db->limit($delimiters['per_page'], $page);
        }

        $this->_query = $this->_ci->db->get($table);
    ...
    protected function _create_object($table, $row)
    {
        ...
        foreach ($row as $column => $value) {
            ...
            foreach ($main_table_information as $table_column) {
                ...
                if ($data && method_exists($model, $mutator)) {
                    $model->$mutator($data);
                }
            }
        ...
        foreach ($table_information as $column) {
            if ($column->is_foreign_key()) {
                if ( ! in_array($column->get_referenced_table(), $tables)) {
                    ...
                    $this->_ci->db->join($column->get_referenced_table() . ' as ' . $foreign_table_alias, $condition, 'left');
                ...
                $this->_ci->db->or_like($table_alias . '.' . $column->get_field(), $keyword);
            }
        }
    }
}