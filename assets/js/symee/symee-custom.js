...
;(function($, window, undefined) {
	...
	$(document).ready(function() {
		...
		$("input.file2[type=file]").each(function(i, el) {
			var $this = $(el),
				label = attrDefault($this, 'label', 'Selecione Imagem');
		...
		if ($.isFunction($.fn.bootstrapWizard)) {
			$(".form-wizard").each(function(i, el) {
				...
				/*
				$(window).on('symee.resize', function() {
					$this.data('bootstrapWizard').show( _index );
				});
				*/
			});
...
function replaceCheckboxes() {
	...
	$(".checkbox-replace:not(.symee-cb-replacement), .radio-replace:not(.symee-cb-replacement)").each(function(i, el) {
		var $this = $(el),
		...
		$this.addClass('symee-cb-replacement');
		$input.wrap($wrapper);
		...
		$input.on('change', function(ev) {	
			if (is_radio) {
				// $(".symee-cb-replacement input[type=radio][name='"+name+"']").closest('.symee-cb-replacement').removeClass(checked_class);
				$(".symee-cb-replacement input[type=radio][name='"+name+"']:not(:checked)").closest('.symee-cb-replacement').removeClass(checked_class);
			}
...
function show_loading_bar(options) {
	...
	var $ = jQuery,
		$loading_bar = $(".symee-loading-bar");
	
	if ($loading_bar.length == 0) {
		$loading_bar = $('<div class="symee-loading-bar progress-is-hidden"><span data-pct="0"></span></div>');
	...
	TweenMax.to($pct, defaults.delay, {
		...
		delay: defaults.wait, ease: is_regress ? Expo.easeOut : Expo.easeIn, 
		onStart: function() {
